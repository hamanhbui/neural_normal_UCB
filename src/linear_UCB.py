...
class LinearUCB:
	...
	def select(self, context):
		ucb = np.array([np.sqrt(np.dot(context[a,:], np.dot(self.A_inv[a], context[a,:].T))) for a in range(7)])
		mu = np.array([np.dot(context[a,:], self.theta[a]) for a in range(7)])
...
if __name__ == '__main__':
	...
	for t in range(min(args.size, b.size)):
		...
		reg = np.max(rwd) - r
		summ+=reg
		loss = l.train(context, arm_select, rwd)
		regrets.append(summ)
	   
	path = "out/logs/linear_UCB"
	fr = open(path,'w')
	for i in regrets:
		fr.write(str(i))
		fr.write("\n")
